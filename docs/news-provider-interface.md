...
The initial browser bundle contains no headlines. After loading the country geometry, the client requests current reporting for every country in small concurrent map-scope batches and explicitly bypasses stored feed reads. It shows progress until each country has a nonempty live result. The resulting multi-event country feeds are ready before the interface becomes interactive; selecting a country then requests a deeper focused feed. Successful results still populate the shared cache for focused country and event queries and provider-failure resilience.
