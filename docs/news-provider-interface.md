...
After loading the country geometry, the client verifies an initial tranche and then exposes the interface while a stable batched sweep checks every remaining map area. Articles must explicitly reference the requested country before they can enter its feed. Each batch supplies both the map signal and the country panel, avoiding a duplicate all-country hydration pass. Matching records from map, selected-country, global, and event searches are merged and rescored as one canonical occurrence. Fresh responses are reused immediately; recent stored map results can be shown while they refresh, and failed fresh requests fall back to the last still-valid snapshot. Failed or omitted countries receive bounded automatic retries using stable batch identities, followed by a focused country search. Selecting a completed country is entirely local; selecting an unresolved country schedules a non-blocking priority lookup.
