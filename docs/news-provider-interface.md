...
The initial browser bundle contains no headlines. After loading the country geometry, the client requests current reporting for every country in concurrent map-scope batches. The interface opens after the first completed batch or three seconds, while the remaining live searches continue visibly in the background. Recent live map results may be reused for up to five minutes; older stored map results are skipped and rebuilt. Country discovery uses a broad current-news search followed by a latest-news search; empty results are retried automatically with backoff rather than filled with unrelated headlines. Selecting a country then requests a deeper multi-provider feed.
