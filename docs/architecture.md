...
Startup keeps the staged readiness screen visible while one persisted whole-world snapshot, the global feed, globe runtime, country geometry, and capital index load in parallel. The browser compiles the snapshot once and reveals the interface only after every country has a verified signal or truthful terminal neutral state and the globe has rendered. If the snapshot is cold or incomplete, only missing countries use bounded batches of up to twelve with six concurrent workers and one forced retry. The completed index populates both the color signal and country panel, so selecting a country does not trigger a second discovery path or mutate its color. A non-blocking warming request refreshes one persisted snapshot slice for future visitors, and recent valid signals survive transient provider failures.
